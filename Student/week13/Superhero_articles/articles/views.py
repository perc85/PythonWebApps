...
from .forms import ArticleForm, InvestigatorForm, SuperheroForm, SuperheroImageFormSet
from django.contrib import messages
...
def superhero_create(request):
    if request.method == 'POST':
        ...
        formset = SuperheroImageFormSet(request.POST, request.FILES)
        if form.is_valid() and formset.is_valid():
            superhero = form.save(commit=False)
            superhero.user = request.user
            superhero.save()
            formset.instance = superhero
            formset.save()
            messages.success(request, "Superhero added successfully!")
            return redirect('superhero_list')
    else:
        form = SuperheroForm()
        formset = SuperheroImageFormSet()
    return render(request, 'superhero_form.html', {'form': form, 'formset': formset})
...
def superhero_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        formset = SuperheroImageFormSet(request.POST, request.FILES, instance=superhero)
        if form.is_valid() and formset.is_valid():
            form.save()
            formset.save()
        ...
        formset = SuperheroImageFormSet(instance=superhero)
    return render(request, 'superhero_form.html', {'form': form, 'formset': formset})
